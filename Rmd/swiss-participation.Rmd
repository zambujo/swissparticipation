...
contribution_share_pillar <- df %>%
  # exclude (country_code == "CH" & call_year == 2014)
...
### Time Distribution of the Contribution to Research & HigherEd

```{r, time-distribution, fig.asp=1}
pal <- c("firebrick4", "gray50", "gray65")
ch_uk <- df %>%
  # remove international organisations
  filter(!str_detect(legal_name, "EUROPEAN |WORLD | MONDIALE")) %>%
  # keep CH and the UK
  filter(country_code %in% c("CH", "UK")) %>%
  filter(org_type == "Research & Education") %>%
...
ch_uk %>%
  bind_rows(rest) %>%
  mutate(Country = factor(Country, levels = c("Switzerland", "United Kingdom", "Remaining"))) %>%
  # filter(Semester < as.Date("2021-01-01")) %>%
  ggplot(aes(x = Semester, y = Share)) +
  geom_col(aes(fill = Country)) +
  facet_grid(vars(Country)) +
  labs(
    title = "Relative Distribution of EU Contribution to Research and HigherEd",
    subtitle = "Comparing Switzerland (Top), the United Kingdom (Center), and the Rest (Bottom)",
    x = "Semester",
    y = "%"
  ) +
  scale_fill_manual(values = pal, guide = "none") +
...
df %>%
  ...
  summarise(
    `Nb Projects` = n_distinct(project_nbr),
    `EU Contribution` = sum(eu_contribution)
  ) %>%
