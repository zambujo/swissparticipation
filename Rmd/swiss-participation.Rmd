...
df <- df %>%
  mutate(
    ...
    legal_entity_type = str_replace(legal_entity_type, fixed("PRC"), "Others (Excl. R&Ed)"),
    legal_entity_type = str_replace(legal_entity_type, fixed("PUB"), "Others (Excl. R&Ed)"),
    legal_entity_type = str_replace(legal_entity_type, fixed("OTH"), "Others (Excl. R&Ed)"),
...
df %>%
  ...
  # sum over other
  group_by(Country) %>%
  summarise(`EU Contribution` = sum(`EU Contribution`, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(desc(`EU Contribution`)) %>%
  mutate(
    Country = forcats::fct_rev(forcats::fct_inorder(Country)),
    Country = forcats::fct_relevel(Country, "Other", after = 0),
    perc = scales::percent(
      `EU Contribution` / sum(`EU Contribution`),
      accuracy = .1,
      trim = FALSE
    )
  ) %>%
  ...
  ggplot(aes(x = `EU Contribution`, y = Country, fill = Country)) +
  geom_col() +
  geom_text(
    aes(label = perc),
    hjust = 1.1,
  ...
  labs(x = "EU Contribution to CH (MioEUR)",
       y = NULL) +
  scale_fill_manual(values = pal, guide = "none") +
  scale_x_continuous(labels = unit_format(
    suffix = "",
    scale = 1e-6,
    big.mark = ","
  )) +
...
```{r ch-type-role, fig.asp=.7, fig.retina=TRUE}
df %>%
  filter(country_code == "CH") %>%
  mutate(
    pillar_abbr = str_replace(pillar_abbr, fixed("Cross-theme"), "Other"),
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Excellent Science is the primary source of funding for CH",
    subtitle = "In which Research & Education organisations represent a high share",
    x = "Pillar",
    y = "EU Contribution in H2020 (MioEUR)"
  ) +
  theme(
...
**Note**: EU Contribution to Switzerland include `r str_c("€", total_cern)` awarded to the CERN.  Please see the EU contribution to CERN detailed below:
