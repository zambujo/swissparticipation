...
Switzerland ranks `r ch_rank`th by total EU contribution in H2020 (incl. the CERN, see [Annex: CERN](#cern)).
...
contribution_share_pillar <- df %>%
  ...
  filter(
    country_code != "CH" |
      (!str_detect(legal_name, "EUROPEAN |WORLD | MONDIALE"))) %>%
...
### CH-UK: EU Contribution to Research & HigherEd

```{r}
pal <- c("firebrick4", "gray50")
df %>%
  ...
  labs(
    ...
    subtitle = "Comparing Switzerland (Top) and the United Kingdom (Baseline)",
    x = "Semester",
    y = "%"
  ) +
  scale_fill_manual(values = pal, guide = "none") +
