...
df %>%
  ...
  select(
    ...
    org_type = legal_entity_type,
    contrib = total_eu_contrib) %>%
  mutate(contrib = round(contrib / 1000000, 1)) %>%
  pivot_wider(names_from = org_type, values_from = contrib, values_fill = 0) %>%
  arrange(Pillar, desc(HES)) %>%
  select(
    Pillar,
    Priority,
    HighEd = HES,
    ...
    Private = PRC,
    Other = OTH) %>%
  write_csv(here("data", "h2020-org-type.csv"))
