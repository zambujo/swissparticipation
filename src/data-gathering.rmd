...
- [data.europa.eu](https://data.europa.eu), **updated 2022-01-18**
  ...
  - [reference data](https://data.europa.eu/data/datasets/cordisref-data)
- [webgate.ec.europa.eu/dashboard (Horizon Dashboard)](https://webgate.ec.europa.eu/dashboard), **updated 2021-12-06**
...
The EU publishes snapshots of its public data in a [data portal](https://data.europa.eu).  The data are available for FP6, FP7, and H2020. For FP7 and H2020, the latest data are packed in ZIP archives which contain CSV files on projects and participants.

First, we check when were the files last modified.

```{r}
source(here::here("R", "package.r"))
source(here::here("R", "utils.r"))

end_points <- here("inst", "urls.txt") %>%
  read_lines()

# FP6 last modified on 09 Mar 2020
last_modified(end_points[1])
last_modified(end_points[2])

# FP7 and H2020 last modified on 18 Jan 2022
last_modified(end_points[3])
last_modified(end_points[4])
```

Second, we run `inst/download.sh` to download the ZIP files to `data-raw/`.

```{r download, comment='', eval=TRUE, echo=FALSE}
cat(readLines(here::here("inst", "download.sh")), sep = '\n')
```

Third, we unzip the archives.
...
[Horizon Dashboard](https://webgate.ec.europa.eu/dashboard) consists of a Qlik Sense application. The raw data is used in official EU statistics but cannot be automatically downloaded. It is possible, nevertheless, to manually export a couple of views to Excel (see [Data sources](#data-sources)).

## Data Preprocessing

### Project Details

Horizon Dashboard project funding data only covers FP7 and H2020. The data is provided in two separate tables, one for each framework programme. To combine both tables, we select the relevant columns:
